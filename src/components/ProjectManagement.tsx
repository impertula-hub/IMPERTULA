...
export function ProjectManagement({ onFormStateChange }: ProjectManagementProps = {}) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Validaciones robustas con funciones de validación
      const tituloValidation = validateTextField(formData.titulo || "", "El título");
      if (!tituloValidation.valid) {
        toast.error(tituloValidation.error);
        return;
      }

      const ubicacionValidation = validateTextField(formData.ubicacion || "", "La ubicación");
      if (!ubicacionValidation.valid) {
        toast.error(ubicacionValidation.error);
        return;
      }
      ...
      const clienteValidation = validateTextField(formData.cliente || "", "El cliente");
      if (!clienteValidation.valid) {
        toast.error(clienteValidation.error);
        return;
      }

      const duracionValidation = validateTextField(formData.duracion || "", "La duración");
      if (!duracionValidation.valid) {
        toast.error(duracionValidation.error);
        return;
      }

      const areaValidation = validateTextField(formData.area || "", "El área");
      if (!areaValidation.valid) {
        toast.error(areaValidation.error);
        return;
      }

      const equipoValidation = validateTextField(formData.equipo || "", "El equipo");
      if (!equipoValidation.valid) {
        toast.error(equipoValidation.error);
        return;
      }

      const descripcionValidation = validateTextField(formData.descripcionCorta || "", "La descripción corta");
      ...
      if (isCreating) {
        ...
        // Recargar proyectos y resetear estado completamente
        await loadProjects();
        confirmCancel(); // Usar confirmCancel() directamente para evitar el diálogo de confirmación
      } else if (editingId) {
        const { _id, id, ...projectData } = formData as any;
    ...
    <>
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => !open && handleCancel()}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl">{isCreating ? "Crear Nuevo Proyecto" : "Editar Proyecto"}</DialogTitle>
          </DialogHeader>
